...
class ShoppingCartProductRepository:
    ...
    def get_all(self):
        try:
            with self.session_factory() as session:
                shopping_cart_products=session.query(ShoppingCartProduct).all()
    ...
    def get_product_in_cart(self, shopping_cart_id, product_id):
        try:
            with self.session_factory() as session:
                item = session.query(ShoppingCartProduct).filter_by(
                    shopping_cart_id=shopping_cart_id,
                    product_id=product_id
                ).one_or_none()
                return item
        except SQLAlchemyError as e:
            print(f"Error fetching product in cart: {e}")
            return None
