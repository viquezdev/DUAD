...
from cache_utils.cart_keys import generate_cache_active_cart_key, generate_cache_carts_all_key
...
@roles_required()
@verify_cache(cache_manager,key_func=lambda user_id: generate_cache_active_cart_key(
    get_jwt_identity()["sub"],
    "admin" if get_jwt_identity()["is_admin"] else "user"
),time_to_live=600)
...
def create_cart():
    try:
        ...
        if not new_shopping_cart:
            return jsonify({
                "error": "Error creating shopping cart"
            }), 400

        owner_user_id = new_shopping_cart.user_id
        requester_role = (
                    "admin"
...
def update_cart(id):
    try:
        ...
        cache_key_cart = generate_cache_active_cart_key(
            owner_user_id, requester_role
        )
...
def delete_cart(id):
    try:
        ...
        cache_key_cart_owner = generate_cache_active_cart_key(
            owner_user_id,
            requester_role,
        )
