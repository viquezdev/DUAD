  ...
  for (const key in student) {
    if (key === "name") {
      ...
      for (const subject of student.grades) {
        if (subject.grade > highesGrade) {
          highesGrade = subject.grade;
          highesGradeName = subject.name;
        }
        if (subject.grade < lowestGrade) {
          lowestGrade = subject.grade;
          lowestGradeName = subject.name;
        }

        sum = sum + subject.grade;
      }
      average = sum / student.grades.length;
      result["gradeAvg"] = average;
      result["highesGrade"] = highesGradeName;
      result["lowestGrade"] = lowestGradeName;
    }
  }
  return result;
}

prueba = studentInformation(student);
console.log(prueba);
